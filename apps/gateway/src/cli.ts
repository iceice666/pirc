...
Commands:
  gateway   Run the gateway / central daemon (configured via PIRC_* environment)
  node      Run a remote node agent that connects out to PIRC_DAEMON_URL
...
switch (command) {
  case 'gateway':
    await (await import('./gateway-main.js')).runGateway();
    break;
  case 'agent':
